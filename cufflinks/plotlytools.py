...
def _to_iplot(self, colors=None, colorscale=None, kind='scatter', mode='lines', interpolation='linear', symbol='dot', size='12', fill=False,
    ...
    if asTimestamp:
        ...
        x = ['({0})'.format(','.join([str(__) for __ in _]))
    ...
    for key in keys:
        ...
        if text is not None:
            lines[key]["text"] = text
        if 'bar' in kind:
            lines[key]["marker"] = {'color': to_rgba(colors[key], .6), 'line': {
                'color': colors[key], 'width': 1}}
        else:
            lines[key]["line"] = {'color': colors[key], 'width': width[key],
                                  'dash': dash[key], 'shape': interpolation[key]}
            ...
            if fill:
                ...
                lines[key]["fillcolor"] = to_rgba(
                    colors[key], kwargs['opacity'] if 'opacity' in kwargs else .3		)
    if 'bar' in kind:
    ...
    if bestfit:
        ...
        bestfit_lines = pd.DataFrame(d).to_iplot(
    ...
    if mean:
        ...
        for key in keys:
            mean = df[key].mean()
            d['MEAN({key})'.format(key=key)] = pd.Series(
                [mean]*len(df[key]), index=df[key].index)
        mean_lines = pd.DataFrame(d).to_iplot(
            mean=False, colors=mean_colors, kind='scatter', asTimestamp=asTimestamp)
...
|          run horizontally
    ...
    # Valid Kwargs
    valid_kwargs = ['color', 'opacity', 'column', 'columns',
                    'labels', 'text', 'world_readable', 'colorbar']
    TRACE_KWARGS = ['hoverinfo', 'connectgaps']
    HEATMAP_SURFACE_KWARGS = ['center_scale', 'zmin', 'zmax']
    PIE_KWARGS = ['sort', 'pull', 'hole', 'textposition',
                  'textinfo', 'linecolor', 'linewidth', 'textcolor']
    OHLC_KWARGS = ['up_color', 'down_color', 'open', 'high',
                   'low', 'close', 'volume', 'name', 'decreasing', 'increasing']
    ...
    if 'column' in kwargs:
        keys = [kwargs['column']] if isinstance(
            kwargs['column'], str) else kwargs['column']
    if 'columns' in kwargs:
        keys = [kwargs['columns']] if isinstance(
            kwargs['columns'], str) else kwargs['columns']
        ...
        if 'line' in dict_modifiers:
            if 'color' not in dict_modifiers['line']:
                if 'linecolor' in tools.getTheme(theme=theme):
                    linecolor = normalize(
                        tools.getTheme(theme=theme)['linecolor'])
                else:
                    linecolor = tools.getLayout(theme=theme)[
                        'xaxis1']['titlefont']['color']
                dict_modifiers['line']['color'] = linecolor
            dict_modifiers['line'] = tools.updateColors(dict_modifiers['line'])
            marker['line'] = deep_update(
                marker['line'], dict_modifiers['line'])
        return marker
    ...
    if not layout:
        l_kwargs = dict([(k, kwargs[k])
                         for k in tools.__LAYOUT_KWARGS if k in kwargs])
        if annotations:
            ann_kwargs = check_kwargs(
                kwargs, tools.__ANN_KWARGS, clean_origin=True)
            annotations = tools.getAnnotations(
                self.copy(), annotations, kind=kind, theme=theme, **ann_kwargs)
            ...
            if kind in ('scatter', 'spread', 'ratio', 'bar', 'barh', 'area', 'line'):
                ...
                if kind in ('spread', 'ratio'):
                    if kind == 'spread':
                        ...
                        positive = trace.apply(
                            lambda x: x if x >= 0 else pd.np.nan)
                        negative = trace.apply(
                            lambda x: x if x < 0 else pd.np.nan)
                        trace = pd.DataFrame(
                            {'positive': positive, 'negative': negative})
                        trace = trace.to_iplot(
                            colors={'positive': 'green', 'negative': 'red'}, width=0.5)
                    else:
                        trace = self.apply(
                            lambda x: x[0]*1.0/x[1], axis=1).to_iplot(colors=['green'], width=1)
                    ...
                    layout['yaxis2'].update(
                        domain=[0, .25], title=kind.capitalize())
                ...
                trace = go.Scatter(x=x, y=y, marker=marker,
                ...
                for _ in columns:
                    if kind == 'box':
                        ...
                        __ = go.Histogram(x=df[_].values.tolist(), name=_,
                                          marker=go.Marker(
                                              color=clrs[_], line=go.Line(width=width)),
                        ...
                        if bins:
                            if type(bins) in (tuple, list):
                                try:
                                    _bins = {
                                        'start': bins[0], 'end': bins[1], 'size': bins[2]}
                                    if orientation == 'h':
                ...
                scale = get_scales(
                    'rdbu') if not colorscale else get_scales(colorscale)
                ...
                if kind == 'heatmap':
                    data = go.Data(
                        [go.Heatmap(z=z, x=x, y=y, zmin=zmin, zmax=zmax, colorscale=colorscale)])
                else:
                    data = go.Data(
                        [go.Surface(z=z, x=x, y=y, zmin=zmin, zmax=zmax, colorscale=colorscale)])
                ...
                marker = go.Marker(colors=get_colors(
                    colors, colorscale, labels, asList=True))
                marker.update(line=go.Line(color=kwargs.pop(
                    'linecolor', None), width=kwargs.pop('linewidth', width)))
                pie = go.Pie(values=values, labels=labels,
                             name='', marker=marker)
                ...
                if len(list(d.keys())) != 4:
                    raise Exception(
                        "OHLC type of charts require an Open, High, Low and Close column")
                ohlc_kwargs = check_kwargs(kwargs, OHLC_KWARGS)
                if kind == 'old_candle':
                    fig = tools.get_candle(
                        self, theme=theme, layout=layout, **ohlc_kwargs)
                else:
                    fig = tools.get_ohlc(
                        self, theme=theme, layout=layout, **ohlc_kwargs)
                if bestfit:
                    df = self.copy()
                    bf = _to_iplot(
                        self[d['close']], bestfit=True, bestfit_colors=bestfit_colors, asTimestamp=True)
                ...
                _d['increasing'] = dict(line=dict(
                    color=kw['up_color']) if 'up_color' in kw else dict(), showlegend=showlegend)
                _d['decreasing'] = dict(line=dict(
                    color=kw['down_color']) if 'down_color' in kw else dict(), showlegend=showlegend)
                ...
                if kind == 'choropleth':
                    if not all([x != None for x in (locations, z)]):
                        raise Exception(
                            "Choropleth maps require a 'location' and 'z' column names specified")
                    ...
                    if not all([x != None for x in (lon, lat)]):
                        raise Exception(
                            "Scattergeo maps require a 'lon' and 'lat' column names specified")
                    geo_data = {'type': 'scattergeo', 'lat': self[lat], 'lon': self[lon],
                                'marker': get_marker(go.Marker(line=go.Line(width=width),
                                                               symbol=symbol, colorscale=get_colorscale(
                                                                   colorscale),
...
# Subplots
    ...
    if subplots:
        ...
        figure = tools.subplots(
            fig, shape, base_layout=layout, theme=theme, **kw)
    ...
    if type(items) != dict:
        if type(items) == list:
            if len(items) != len(keys):
                raise Exception(
                    'List of {0} is not the same length as keys'.format(items_names))
            else:
...
def _scatter_matrix(self, theme=None, bins=10, color='grey', size=2, asFigure=False, **iplot_kwargs):
    ...
    """
    sm = tools.scatter_matrix(
        self, theme=theme, bins=bins, color=color, size=size)
    if asFigure:
        return sm
    else:
...
def _ta_plot(self, study, periods=14, column=None, include=True, str='{name}({period})', detail=False,
    ...
    if theme is None:
        theme = iplot_kwargs.pop(
            'study_theme', auth.get_config_file()['theme'])
    ...
    def get_subplots(figures):
        ...
        subplots = tools.subplots(
            figures, shape=shape, shared_xaxes=True, base_layout=layout)
        if len(figures) == 2:
    ...
    def get_study(df, func, iplot_kwargs, iplot_study_kwargs, str=None, include=False, column=None, inset=False):
        ...
        iplot_study_kwargs['colors'] = iplot_study_kwargs.get(
            'colors', ['blue', 'green', 'red'] if study == 'dmi' else 'blue')
    ...
    study_kwargs = check_kwargs(
        iplot_kwargs, __TA_KWARGS, {}, clean_origin=True)
