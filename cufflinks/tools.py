...
def getLayout(kind=None, theme=None, title='', xTitle='', yTitle='', zTitle='', barmode='', bargap=None, bargroupgap=None,
    ...
    if is3d:
        ...
        scene = go.Scene(xaxis=layout['xaxis1'].copy(
    ...
    for r in ['x', 'y', 'z']:
        if '{0}range'.format(r) in kwargs:
            if is3d:
                layout['scene']['{0}axis'.format(r)].update(
                    range=kwargs['{0}range'.format(r)])
            else:
                layout['{0}axis1'.format(r)].update(
                    range=kwargs['{0}range'.format(r)])
        ...
        if type(kwargs['legend']) == bool:
            ...
            if kwargs['legend'] == 'top':
                layout['legend'].update(
                    orientation='h', yanchor='bottom', x=.3, y=.95)
            elif kwargs['legend'] == 'bottom':
                layout['legend'].update(
                    orientation='h', yanchor='bottom', x=.3, y=-0.5)
    ...
    if any(k in kwargs for k in ['vline', 'hline', 'shapes', 'hspan', 'vspan']):
        ...
        def get_span(xspan):
            ...
            if isinstance(xspan, list):
                ...
                v0, v1 = xspan
                if orientation == 'h':
                    shapes.append(get_shape(kind='rect', y0=v0,
                                            y1=v1, fill=True, opacity=.5))
                else:
                    shapes.append(get_shape(kind='rect', x0=v0,
                                            x1=v1, fill=True, opacity=.5))
            else:
                raise Exception(
                    'Invalid value for {0}span: {1}'.format(orientation, xspan))
    ...
    if kind in ('choropleth', 'scattergeo'):
        ...
        defaults = {'projection': {'type': 'equirectangular'},
    ...
    if 'rangeslider' in kwargs:
        if type(kwargs['rangeslider']) == bool:
            if kwargs['rangeslider']:
                layout['xaxis1']['rangeslider'] = dict(
                    visible=kwargs['rangeslider'])
            else:
...
def getAnnotations(df, annotations, kind='lines', theme=None, **kwargs):
    ...
    kwargs['fontcolor'] = kwargs.pop(
        'fontcolor', theme_data['annotations']['fontcolor'])
    kwargs['arrowcolor'] = kwargs.pop(
        'arrowcolor', theme_data['annotations']['arrowcolor'])
    ...
    def check_ann(annotation):
        ...
        try:
            ...
            _annotation = dict_replace_keyword(
                _annotation, 'font', kwargs, False)
            ...
            if 'title' in annotation:
                local_list.append(
                    go.Annotation(
                        ...
                        yref='paper',
                        font={
                            'size': 24 if not 'fontsize' in kwargs else kwargs['fontsize']}
                    )
                )
...
def subplots(figures, shape=None,
    ...
    for d in data:
        if d['type'] == 'pie':
            d['domain'] = {}
            d['domain']['x'] = layout['xaxis{0}'.format(
                d['xaxis'][1:])]['domain']
            d['domain']['y'] = layout['yaxis{0}'.format(
                d['yaxis'][1:])]['domain']
    return sp
...
def scatter_matrix(df, theme=None, bins=10, color='grey', size=2):
    ...
    for i in df.columns:
        for j in df.columns:
            if i == j:
                fig = df.iplot(kind='histogram', keys=[
                               i], asFigure=True, bins=bins)
                figs.append(fig)
            else:
                figs.append(df.iplot(kind='scatter', mode='markers',
                                     x=j, y=i, asFigure=True, size=size, colors=[color]))
...
def trace_dict(self):
    ...
    for trace in range(len(self['data'])):
        ...
        name = '{0}_'.format(trace_['name']) if trace_[
            'name'] in d else trace_['name']
        d[name] = trace
    return d
...
def get_ref(figure):
    ...
    for trace in figure['data']:
        name = '{0}_'.format(
            trace['name']) if trace['name'] in d else trace['name']
...
def get_def(figure):
    d = {}
    items = list(figure['layout']['scene'].items(
    )) if 'scene' in figure['layout'] else list(figure['layout'].items())
    for k, v in items:
        if 'axis' in k:
...
def get_len(figure):
    d = {}
    keys = list(figure['layout']['scene'].keys(
    )) if 'scene' in figure['layout'] else list(figure['layout'].keys())
    for k in keys:
        if 'axis' in k:
...
def get_dom(figure):
    ...
    for k in ('x', 'y'):
        ...
        for y in which[k]:
            item = axis_def[y]
            domain = '[0.0, 1.0]' if 'domain' not in item else str(
                item['domain'])
...
def _set_axis(self, traces, on=None, side='right', title=''):
    ...
    for trace in traces:
        if on:
            ...
            domain = '[0.0, 1.0]' if 'domain' not in fig.axis['def'][curr_y] else str(
                fig.axis['def'][curr_y]['domain'])
            try:
                new_axis = fig.axis['dom']['y'][domain][side]
            except KeyError:
                axis = go.YAxis(fig.axis['def'][curr_y].copy())
                # check overlaying values
                axis.update(title=title, overlaying=curr_y,
                            side=side, anchor=curr_x)
...
def get_error_bar(axis='y', type='data', values=None, values_minus=None, color=None, thickness=1, width=5,
        ...
        if isinstance(values, list) or isinstance(values, np.ndarray):
            ...
            if values_minus:
                if isinstance(values_minus, list) or isinstance(values_minus, np.ndarray):
                    raise Exception(
                        'Values should be of same type (int, float)')
    ...
    error.update(type=type, thickness=thickness,
                 width=width, visible=True, opacity=opacity)
    if color:
...
def set_errors(figure, trace=None, axis='y', type='data', values=None, values_minus=None, color=None, thickness=1, width=None,
    ...
    if 'continuous' not in type:
        ...
        if trace:
            data[figure.trace_dict[trace]].update(
                {'error_{0}'.format(axis): error})
        else:
            for trace in data:
        ...
        def get_traces(trace, value, type, color=None, width=.2, opacity=.3):
            ...
            lower.update(fill='tonexty', fillcolor=color,
                         name=name+'lower', y=y_down)
            return upper, lower
        if trace:
        ...
        for i in traces:
            trace = figure['data'][i]
            upper, lower = get_traces(
                trace, values, type, color=color, width=width, opacity=opacity)
...
def _move_axis(self, xaxis=None, yaxis=None):
    def update_axis(self, axis):
        _axis = axis[0]
        from_axis = self.data[0].pop(
            '{0}axis'.format(_axis), '{0}1'.format(_axis))
