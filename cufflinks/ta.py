...
def rsi(df, periods=14, column=None, include=True, str='{name}({column},{period})', detail=False, **kwargs):
    ...
    __df = pd.concat([_rsi(df, column=x, periods=y, include=False,
                           str=str, detail=detail) for y in periods for x in column], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
        return __df
...
def sma(df, periods=21, column=None, include=True, str='{name}({column},{period})', detail=False):
    ...
    column = make_list(column)
    periods = make_list(periods)
    __df = pd.concat([_sma(df, column=x, periods=y, include=False, str=str)
                      for y in periods for x in column], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
        return __df
...
def ema(df, periods=21, column=None, include=True, str='{name}({column},{period})', detail=False):
    def _ema(df, periods, column, include, str, detail=False):
        ...
        # === pure python ====
        _df['EMA'] = df[column].ewm(
            span=periods, min_periods=periods, adjust=False).mean()
        # === /pure python ====

        return rename(df, _df, study, periods, column, include, str, detail)
    column = make_list(column)
    periods = make_list(periods)
    __df = pd.concat([_ema(df, column=x, periods=y, include=False, str=str)
                      for y in periods for x in column], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
...
def adx(df, periods=14, high='high', low='low', close='close', di=False, include=True, str='{name}({period})', **kwargs):
    def _adx(df, periods, high, low, close, include, str, detail):
        ...
        def smooth(col):
            ...
            for _ in list(range(periods+1, len(col))):
                sm.iloc[_] = sm.iloc[_-1] - \
                    (1.0*sm.iloc[_-1]/periods)+col.iloc[_]
            return sm
        ...
        __df = pd.DataFrame(
            dict(enumerate([df[high]-df[high].shift(1), df[low].shift(1)-df[low]])))
        _df['DM+'] = __df.apply(lambda x: max(x[0], 0)
                                if x[0] > x[1] else 0, axis=1)
        _df['DM-'] = __df.apply(lambda x: max(x[1], 0)
                                if x[1] > x[0] else 0, axis=1)
        _df.iloc[0] = np.nan
        ...
        _df['DI-'] = 100.0*_df_smooth['DM-']/_df_smooth['TR']
        dx = pd.DataFrame(
            100*abs(_df['DI+']-_df['DI-'])/(_df['DI+']+_df['DI-']))
    ...
    __df = pd.concat([_adx(df, periods=y, high=high, low=low, close=close,
                           include=False, str=str, detail=detail) for y in periods], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
        return __df
...
def atr(df, periods=14, high='high', low='low', close='close', include=True, str='{name}({period})', **kwargs):
    ...
    periods = make_list(periods)
    __df = pd.concat([_atr(df, periods=y, high=high, low=low,
                           close=close, include=False, str=str) for y in periods], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
        return __df
...
def ptps(df, periods=14, initial='long', af=.02, high='high', low='low', include=True, str='{name}({period})', **kwargs):
    def _ptps(df, periods, high, low, include, str, detail):
        ...
        for _ in range(1, len(df)):
            ...
            if _ == 1:
                ...
                _df.loc[_, 'SAR'] = _df['EP'][_-1] if _df['LorS'][_ -
                                                                  1] != _df['LorS'][_] else _df['T_SAR'][_-1]
            ...
            if _df['LorS'][_] == 'long':
                if _df['SAR'][_] >= df[low][_]:
                    ...
                    _df.loc[_, 'EP'] = df[high][_] if df[high][_] > _df['EP'][_ -
                                                                              1] else _df['EP'][_-1]
            else:
                if _df['SAR'][_] <= df[high][_]:
                    _df.loc[_, 'EP'] = df[high][_]
                else:
                    _df.loc[_, 'EP'] = df[low][_] if df[low][_] < _df['EP'][_ -
                                                                            1] else _df['EP'][_-1]
            ...
            if _df['LorS'][_] == 'long':
                if _df['SAR'][_] >= df[low][_]:
                    _df.loc[_, 'T_SAR'] = max(
                        _df['SAR'][_]-_df['AF_Diff'][_], df[high][_], df[high][_-1])
                else:
                    _df.loc[_, 'T_SAR'] = min(
                        _df['SAR'][_]+_df['AF_Diff'][_], df[low][_], df[low][_-1])
            else:
                if _df['SAR'][_] <= df[high][_]:
                    _df.loc[_, 'T_SAR'] = min(
                        _df['SAR'][_]+_df['AF_Diff'][_], df[low][_], df[low][_-1])
        ...
        _df['LONG'] = _df.apply(lambda x: x['T_SAR']
                                if x['LorS'] == 'long' else np.nan, axis=1)
        _df['SHORT'] = _df.apply(
            lambda x: x['T_SAR'] if x['LorS'] == 'short' else np.nan, axis=1)
    ...
    periods = make_list(periods)
    output = ['LONG', 'SHORT']
    __df = pd.concat([_ptps(df, periods=y, high=high, low=low,
                            include=False, str=str, detail=detail) for y in periods], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
...
def cci(df, periods=14, high='high', low='low', close='close', include=True, str='{name}({period})', **kwargs):
    def _cci(df, periods, high, low, close, include, str, detail=False):
        ...
        return rename(df, _df, study, periods, '', include, str, detail)
    periods = make_list(periods)
    __df = pd.concat([_cci(df, periods=y, high=high, low=low,
                           close=close, include=False, str=str) for y in periods], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
...
def correl(df, periods=21, columns=None, include=True, str=None, detail=False, how='value', **correl_kwargs):
    ...
    def _correl(df, periods=21, columns=None, include=True, str=None, detail=False, **correl_kwargs):
        ...
        _df['CORREL'] = df[columns[0]].rolling(
            window=periods, **correl_kwargs).corr(df[columns[1]])

        str = str if str else 'CORREL({column1},{column2},{period})'.format(
            column1=columns[0], column2=columns[1], period=periods)
        return rename(df, _df, study, periods, columns, include, str, detail)
    columns = df.columns if not columns else columns
    if len(columns) != 2:
        raise StudyError(
            "2 Columns need to be specified for a correlation study")
    periods = make_list(periods)
    if how == 'pct_chg':
        df = df[columns].pct_change()
    elif how == 'diff':
        df = df[columns].diff()
    __df = pd.concat([_correl(df, columns=columns, periods=y,
                              include=False, str=str) for y in periods], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
        return __df
...
def boll(df, periods=20, boll_std=2, column=None, include=True, str='{name}({column},{period})', detail=False, **boll_kwargs):
    def _boll(df, periods, column):
        ...
        _df['UPPER'] = _df['SMA'] + \
            df[column].rolling(window=periods).std()*boll_std
        _df['LOWER'] = _df['SMA'] - \
            df[column].rolling(window=periods).std()*boll_std
        # === /pure python ====
    ...
    __df = pd.concat([_boll(df, column=x, periods=y)
                      for y in periods for x in column], axis=1)
    if include:
        return pd.concat([df, __df], axis=1)
    else:
...
def macd(df, fast_period=12, slow_period=26, signal_period=9, column=None, include=True, str=None, detail=False, **macd_kwargs):
    ...
    output = ['MACD', 'SIGNAL']
    __df = pd.concat([_macd(df, column=x, include=False)
                      for x in column], axis=1)
